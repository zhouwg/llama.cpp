...
enum ggml_status ggml_backend_graph_compute(ggml_backend_t backend, struct ggml_cgraph * cgraph) {
    ...
    if (backend != g_cpu_backend) {
        if (is_qnn_backend(backend)) { // or if ggml_backend_buft_is_host return true
...
bool ggml_backend_offload_op(ggml_backend_t backend, const struct ggml_tensor * op) {
    if (is_qnn_backend(backend)) { // or if ggml_backend_buft_is_host return true, used for compatible for "Backend Sched" feature
        return false;
    }
